...
export default class InfixExpression {
  ...
  get list(): readonly (Operand | Operator)[] {
  ...
  private _generateParams(
    startPos: number,
    symbols: MathSymbol[]
  ): { params: (Operand | Operator)[][]; endPos: number } {
    let opLvl = 1,
      pos = startPos,
      param: MathSymbol[] = [];
    const params: (Operand | Operator)[][] = [];
    while (pos < symbols.length) {
      pos += 1;
  ...
  private _pushOperator(infixList: (Operand | Operator)[], operator: Operator) {
  ...
  private _pushOperand(infixList: (Operand | Operator)[], operand: Operand) {
    if (operand.length == 0) {
      return;
    }
    const prevItem = infixList[infixList.length - 1];
